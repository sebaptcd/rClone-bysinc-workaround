	...
	if ping -c 1 8.8.8.8 &> /dev/null; then
		...
		# Start new syncing instance.
			...
			if [ -f "$boot_lock" ]; then
				...
				#
				# rclone sync --create-empty-src-dirs --update --progress --log-level DEBUG | server -> local
				#
				...
				while true; do
					...
					if ping -c 1 8.8.8.8 &> /dev/null; then
						...
						# Personal
						rclone sync --create-empty-src-dirs --update --progress --log-level DEBUG "$Personal_remote" "$Persoanl_local" | while read line; do
						# Check for errors
						if [[ "$line" = "Errors:" ]]; then
						# Send a notification
						...
						while true; do
							...
							if ping -c 1 8.8.8.8 &> /dev/null; then
								...
								# Dev
								rclone sync --create-empty-src-dirs --update --progress --log-level DEBUG "$Dev_remote" "$Dev_local" | while read line; do
								# Check for errors
								if [[ "$line" = "Errors:" ]]; then
								# Send a notification
								notify-send "Error: $line"
								...
								while true; do
									...
									if ping -c 1 8.8.8.8 &> /dev/null; then
										...
										# WORKSTATION
										rclone sync --create-empty-src-dirs --update --progress --log-level DEBUG "$WORKSTATION_remote" "$WORKSTATION_local" | while read line; do
										# Check for errors
										if [[ "$line" = "Errors:" ]]; then
										# Send a notification
										notify-send "Error: $line"
											...
											while true; do
												...
												# Check if the internet connection is available
													...
													if [ ! -f "$sync_lock" ]; then
														...
														#
														# rclone sync --create-empty-src-dirs --update --progress --log-level DEBUG | local -> server
														#
														...
														while true; do
															...
															if ping -c 1 8.8.8.8 &> /dev/null; then
																...
																# Personal
																rclone sync --create-empty-src-dirs --update --progress --log-level DEBUG "$Persoanl_local" "$Personal_remote" | while read line; do
																# Check for errors
																if [[ "$line" = "Errors:" ]]; then
																# Send a notification
																notify-send "Error: $line"
																...
																while true; do
																	...
																	if ping -c 1 8.8.8.8 &> /dev/null; then
																		...
																		# Dev
																		rclone sync --create-empty-src-dirs --update --progress --log-level DEBUG "$Dev_local" "$Dev_remote" | while read line; do
																		# Check for errors
																		if [[ "$line" = "Errors:" ]]; then
																		# Send a notification
																		notify-send "Error: $line"
																		...
																		while true; do
																			...
																			if ping -c 1 8.8.8.8 &> /dev/null; then
																				...
																				# WORKSTATION
																				rclone sync --create-empty-src-dirs --update --progress --log-level DEBUG "$WORKSTATION_local" "$WORKSTATION_remote" | while read line; do
																				# Check for errors
																				if [[ "$line" = "Errors:" ]]; then
																				# Send a notification
																				notify-send "Error: $line"
