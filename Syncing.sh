...
#  local code=$?
#  if [ "$code" -ne 0 ]; then
#    notify-send "Syncing" "Sync script failed, exit code $code"
#    echo "Exit $code at $(date)" >> "$Err/errLog$(get_timestamp).txt"
#  fi
#}
#trap 'on_exit' EXIT
...
set -e
handleError() {
  notify-send "Syncing" "Syncing script failed, exiting..."
}
...
source "$script_dir/../0. Private/rClone-bisync-workaround/var.sh"

# Declare the machine ID
machineID=$(cat $Setups/rClone-bisync-workaround/machineID.txt)
...
syncControl_local_lock="$Setups/rClone-bisync-workaround/Locks/syncTime.lock"

# "$Setups" is the local path for the script, declared in "var.sh"
sync_lock="$Setups/rClone-bisync-workaround/Locks/Sync.lock"
boot_lock="$Setups/rClone-bisync-workaround/Locks/Boot-Sync.lock"
boot_ran_lock="$Setups/rClone-bisync-workaround/Locks/Boot-Ran.lock"
logs="$Setups/rClone-bisync-workaround/Logs"
...
while true; do
	...
	if ping -c 1 8.8.8.8 &> /dev/null; then
		...
		if [ -f "$sync_lock" ]; then
			...
			if [ -e "$syncControl_local_lock" ]; then
				rm "$syncControl_local_lock" && echo "deleted $syncControl_local_lock"
			else
				echo "Not found to delete: $syncControl_local_lock"
			fi
			sleep 300
		...
		# Start new syncing instance.
			...
			# Check if Boot Syncing was interrupted mid sync but no longer runs
				...
				echo "Syncing: Checking rClone version..."
				rclone selfupdate --check 2>&1 | {
					...
					# If the version cannot be fetched, skip the update
					elif [[ "$second_line" == *"ERROR"* ]]; then
						echo "Could not fetch latest version — continuing without update."
						notify-send "Syncing" "Could not fetch latest version — continuing without update."
					else
				...
				# Check and log if another syncing process is ongoing on another machine
				{
					while true; do
						...
						if syncControl_lock_value=$(rclone cat "$syncControl_lock" 2>/dev/null); then
							...
							# If the rclone output contains the itemNotFound JSON, restart loop
							if printf '%s\n' "$syncControl_lock_value" | grep -q '"error":.*"code":"itemNotFound"\|Item not found'; then
								echo "rclone returned itemNotFound error — retrying loop from start."
								notify-send "Syncing" "rclone returned itemNotFound error — retrying loop from start."
								sleep 1
								continue
							fi
							...
							# If the lock is not active, delete it.
								...
								echo "Inactive sync lock found."
								rclone delete "$syncControl_lock" || {
									rcExitStatus=$?
									# Accept both rclone 3 or 4 not-found codes.
									if [ "$rcExitStatus" -eq 3 ] || [ "$rcExitStatus" -eq 4 ]; then
										echo "RCLONE returned code 3 or 4 at the start of Boot Sync."
										notify-send "Syncing" "NOTICE: RCLONE returned code 3 or 4 at the start of Boot Sync. Ignoring..."
										true
									else
										echo "Another error was detected at the start of Boot Sync."
										notify-send "Syncing" "ERROR: Script encountered an error on the Sync Control deletion, at the start of Boot Sync."
										exit $rcExitStatus
									fi
								}
								echo "Inactive sync lock deleted."
				...
				# Personal
				rclone sync "${common_flags[@]}" "${sync_flags[@]}" "${dynamic_sync_server_local_flags[@]}" "$Personal_remote" "$Personal_local" 2>&1 | while read line; do
					# Check for errors
				...
				rclone bisync --resync "${common_flags[@]}" "${bisync_flags[@]}" "$Personal_local" "$Personal_remote" 2>&1 | while read line; do
					# Check for errors
				...
				# Personal/Public
				rclone bisync "${common_flags[@]}" "${bisync_flags[@]}" "$PersonalPb_remote" "$PersonalPb_local" 2>&1 | while read line; do
					# Check for errors
					if [[ "$line" == *".conflict"* || "$line" == *"ERROR"* || "$line" == *"Errors:"* ]]; then
						notify-send "Syncing Error:" "$line"
					fi
				...
				# Mark that the syncing process has finished on this machine
				{
					...
					kill $active_sync
					# Give $active_sync time to shut down before moving forward
					sleep 5
					if [ -e "$syncControl_local_lock" ]; then
						rm "$syncControl_local_lock" && echo "Deleted $syncControl_local_lock"
					else
					...
					rclone delete "$syncControl_lock" || {
						...
						# Accept both rclone 3 or 4 not-found codes.
						if [ "$rcExitStatus" -eq 3 ] || [ "$rcExitStatus" -eq 4 ]; then
							echo "RCLONE returned code 3 or 4 on the final processes from Boot Sync."
							notify-send "Syncing" "NOTICE: RCLONE returned code 3 or 4 on the final processes from Boot Sync. Ignoring..."
							true
						else
							echo "Another error was detected on the final processes from Boot Sync."
							notify-send "Syncing" "ERROR: Script encountered an error on the Sync Control deletion, at the final processes from Boot Sync."
							exit $rcExitStatus
						fi
					}
					...
					while true; do
						...
						if ping -c 1 8.8.8.8 &> /dev/null; then
							...
							if [ -f "$boot_lock" ] || [ -f "$boot_ran_lock" ]; then
										...
										if syncControl_lock_value=$(rclone cat "$syncControl_lock" 2>/dev/null); then
											...
											# If the lock is not active, delete it.
												...
												rclone delete "$syncControl_lock" || {
													...
													# Accept both rclone 3 or 4 not-found codes.
													if [ "$rcExitStatus" -eq 3 ] || [ "$rcExitStatus" -eq 4 ]; then
														echo "RCLONE returned code 3 or 4 at the beginning of the Sync Cycle."
														notify-send "Syncing" "NOTICE: RCLONE returned code 3 or 4 at the beginning of the Sync Cycle. Ignoring..."
														true
													else
														echo "Another error was detected at the beginning of the Sync Cycle."
														notify-send "Syncing" "ERROR: Script encountered an error on the Sync Control deletion from the beginning of the Sync Cycle."
														exit $rcExitStatus
													fi
												}
												echo "Inactive sync lock deleted."
								...
								rclone sync "${common_flags[@]}" "${sync_flags[@]}" "${dynamic_sync_server_local_flags[@]}" "$Personal_local" "$Personal_local_backup" 2>&1 | while read line; do
									# Check for errors
									if [[ "$line" == *"ERROR"* || "$line" == *"Errors:"* ]]; then
										notify-send "Syncing Error:" "$line"
									fi
								...
								# Personal
								rclone bisync "${common_flags[@]}" "${bisync_flags[@]}" "$Personal_local" "$Personal_remote" 2>&1 | while read line; do
									# Check for errors
									if [[ "$line" == *".conflict"* || "$line" == *"ERROR"* || "$line" == *"Errors:"* ]]; then
										notify-send "Syncing Error:" "$line"
									fi
								...
								# Mark that the syncing process has finished on this machine
								{
									...
									rclone delete "$syncControl_lock" || {
										...
										# Accept both rclone 3 or 4 not-found codes.
										if [ "$rcExitStatus" -eq 3 ] || [ "$rcExitStatus" -eq 4 ]; then
											echo "RCLONE returned code 3 or 4 at the final process of the Sync Cycle."
											notify-send "Syncing" "NOTICE: RCLONE returned code 3 or 4 at the final processes of the Sync Cycle. Ignoring..."
											true
										else
											echo "Another error was detected at the final processes from the Sync Cycle."
											notify-send "Syncing" "ERROR: Script encountered an error on the Sync Control deletion, at the final processes from the Sync Cycle."
											exit $rcExitStatus
										fi
									}
									echo "syncControl mark removed."
